...
public class ModuleDependencySorterTest {
    ...
    private static ModuleWithDependencies createModuleWithDependencies(String name, Set<ModuleWithDependencies> dependencies) {
        ModuleInformation moduleInformation = new DummyModuleInformation(name, dependencies.stream().map(d -> d.getModuleInformation().getId()).collect(
                Collectors.toSet()));
    ...
    public void sortWithMultipleDependencies() {
        ...
        ModuleWithDependencies moduleB = createModuleWithDependencies("module.b", Collections.emptySet());
        modules.add(moduleB);
        ModuleWithDependencies moduleC = createModuleWithDependencies("module.c", new HashSet<>(Arrays.asList(moduleA, moduleB)));
        modules.add(moduleC);
